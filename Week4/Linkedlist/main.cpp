...
{
    List *newList = new List();
    newList->p_head = newList->p_tail = p_node;
    return newList;
}

bool addHead(List *&L, int data)
{
    if (L->p_head == nullptr) L->p_head= L->p_tail = createNode(data);
    else {
...
void removePos(List *&L, int data, int pos)
{
    ...
    delete p;
    if(prev->p_next == nullptr) L->p_tail = prev;
}
...
void printList(List *L)
{
    if(L->p_head == nullptr) cout << "Empty List" << endl; return;
    NODE *p = L->p_head;
    while (p != nullptr) {
        cout << p->key << " ";
        p = p->p_next;
...
List *reverseList(List *L)
{
    ...
    NODE *p = L->p_head;
    while (p != nullptr) {
        NODE *temp = createNode(p->key);
        temp->p_next = newL->p_head;
        newL->p_head = temp;
        p = p->p_next;
    }
    newL->p_tail = L->p_head;
    return newL;
}
