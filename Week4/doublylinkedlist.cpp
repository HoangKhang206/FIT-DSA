...
bool addHead(d_List *&L, int data)
{
    ...
    if (L->pHead == nullptr) L->pHead= L->pTail = newNode;
    else {
...
void removeBefore(d_List *&L, int val)
{
    ...
    d_NODE *p = L->pHead->pNext;
    while (p!= nullptr && p->key != val) {
        p = p->pNext;
    }
    if(p==nullptr || p->pPrev==nullptr) return;
    d_NODE *before = p->pPrev;
    d_NODE *prev = before->pPrev;
    if(prev==nullptr) {
        removeHead(L);
        return;
    }
...
void removeAfter(d_List *&L, int val)
{
    ...
    if (p == nullptr||p->pNext==nullptr) return;
    d_NODE *after = p->pNext;
    d_NODE *next = after->pNext;
    if(next==nullptr) removeTail(L);
    else {
        p->pNext = next;
        next->pPrev = p;
        delete after;
...
void printList(d_List *L)
{
    if (L->pHead == nullptr) cout << "List is empty" << endl; return;
    d_NODE *p = L->pHead;
    while (p != nullptr) {
        cout << p->key << " ";
        p = p->pNext;
...
d_List *reverseList(d_List *L)
{
    d_List *newL = new d_List();
    d_NODE *p = L->pHead;
    while (p != nullptr) {
        d_NODE *temp = createNode(p->key);
        if (newL->pHead == nullptr) {
            newL->pHead = newL->pTail = temp;
        }
        else {
            temp->pNext = newL->pHead;
            newL->pHead->pPrev = temp;
            newL->pHead = temp;
        }
        p = p->pNext;
    }
...
bool removeElement(d_List *&L, int key)
{
    ...
    if (L->pHead == nullptr) return true;
    d_NODE *prev = L->pHead;
    d_NODE *p = prev->pNext;
    while (p != nullptr) {
        if (p->key == key) {
